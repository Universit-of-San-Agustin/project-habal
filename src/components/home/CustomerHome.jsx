...
import { MapPin, Clock, Star, ChevronRight, User, LogOut, Banknote, Bike, Navigation } from "lucide-react";
...
export default function CustomerHome({ user }) {
  ...
  const [activeRide, setActiveRide] = useState(null); // booking in progress
  ...
  useEffect(() => {
    if (user) {
      ...
      base44.entities.Booking.filter({ customer_phone: user.email }, "-created_date", 1).then(rows => {
  ...
  // Active ride tracker overlays everything
  if (activeRide && !showProfile) {
    return (
      <div className="fixed inset-0 flex flex-col bg-white max-w-md mx-auto overflow-hidden">
        <ActiveRideTracker
          ...
          onDismiss={() => {
            setActiveRide(null);
            setBookings(prev => prev.map(b => b.id === activeRide.id ? { ...b, status: "cancelled" } : b));
          }}
        />
      </div>
    );
    ...
    return (
      <div className="fixed inset-0 flex flex-col bg-white max-w-md mx-auto overflow-hidden">
        <div className="flex-1 overflow-y-auto pt-12">
          <CustomerProfile user={user} onBack={() => setShowProfile(false)} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-white max-w-md mx-auto overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-4 py-3 bg-white/90 backdrop-blur-sm border-b border-gray-100">
        ...
        <button onClick={() => setShowProfile(true)} className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center">
        ...
        <>
          ...
          <div className="absolute top-14 left-0 right-0 z-10 px-3 pt-3 space-y-2">
            <div className="bg-white rounded-2xl shadow-md px-4 py-3 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wider mb-0.5">Pick-up</div>
                <div className="text-sm text-gray-800 font-medium truncate">{pickup || "Set pickup"}</div>
              </div>
            </div>
          ...
          <div className="absolute bottom-14 left-0 right-0 z-10 px-3 pb-3 space-y-2">
            <div className="flex gap-2">
              <button onClick={() => setPaymentMethod(p => p === "Cash" ? "GCash" : "Cash")}
            ...
            <button onClick={handleBook} disabled={booking || !dropoff}
              ...
              <span className="text-base font-semibold">
                {booking ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto" /> : "Order Ride"}
              </span>
            </button>
          </div>
        </>
          ...
          {bookings.length === 0 ? (
            ...
            <div key={b.id} className="bg-white border border-gray-100 rounded-2xl p-4 mb-3 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full capitalize font-medium ${b.status === "completed" ? "bg-emerald-50 text-emerald-600" : b.status === "cancelled" ? "bg-red-50 text-red-500" : "bg-yellow-50 text-yellow-600"}`}>
                  {b.status}
                </span>
              </div>
