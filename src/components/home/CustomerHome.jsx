        ...
        <div className="flex-1 overflow-y-auto px-4 pt-4">
          ...
          {savedLocations.map(loc => (
            <div key={loc.id} className="flex items-center gap-4 bg-white border border-gray-100 rounded-2xl px-4 py-4 mb-3 shadow-sm">
              ...
              <button onClick={() => {
                ...
                setScreen("search");
              }} className="text-xs font-semibold px-3 py-1.5 rounded-xl" style={{ background: PRIMARY_BG, color: PRIMARY_DARK }}>
                Go
              </button>
            </div>
          ))}
          {addingLocation ? (
            <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm space-y-3">
              <div className="text-sm font-bold text-gray-700">New Saved Location</div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={async () => {
                  ...
                  style={{ background: PRIMARY }}>Save</button>
              </div>
            </div>
          ) : (
        ...
        <div className="absolute inset-0 z-20 flex flex-col bg-white">
          <div className="px-4 pt-12 pb-4 border-b border-gray-100">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-bold text-gray-900 text-lg">Select Destination</span>
            </div>
            {/* Pickup */}
            <div className="flex items-center gap-3 px-3.5 py-2.5 bg-gray-50 rounded-xl mb-3 border border-gray-100">
              <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: PRIMARY }} />
              <span className="text-sm font-medium text-gray-700 truncate flex-1">{pickup || "Your location"}</span>
            </div>
            {/* Dropoff input */}
            <div className="flex items-center gap-3 px-3.5 py-2.5 border-2 rounded-xl bg-white" style={{ borderColor: PRIMARY }}>
              ...
              {dropoffInput && (
                <button onClick={() => { setDropoffInput(""); setSuggestions([]); }} className="hover:opacity-70">
          ...
          <div className="flex-1 overflow-y-auto px-4 pt-3">
            ...
            {dropoffInput.length >= 3 && suggestions.length === 0 && (
              <div className="text-center py-12">
                ...
                <div className="text-xs text-gray-300 mt-1">Try a different search</div>
              </div>
            )}
            {/* Popular places */}
            {!dropoffInput && (
              <div className="pb-4">
                ...
                <div className="space-y-2">
                  {[
                    { name: "SM City Iloilo", icon: "🏬", sub: "Shopping Mall" },
                    { name: "Robinsons Place Iloilo", icon: "🛍", sub: "Shopping Mall" },
                    { name: "Iloilo Business Park", icon: "🏢", sub: "Business District" },
                    { name: "Fort San Pedro", icon: "🏰", sub: "Historical Site" },
                    { name: "Iloilo City Hall", icon: "🏛", sub: "Government" },
                  ].map(place => (
                    <button key={place.name} onClick={() => selectSuggestion({ place_name: place.name, center: [122.56, 10.69], place_type: "poi" })}
                      className="w-full flex items-center gap-3 px-3 py-3 bg-white border border-gray-100 rounded-xl hover:bg-gray-50 hover:shadow-sm transition-all text-left">
